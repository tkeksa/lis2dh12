...
impl<I2C: I2c> Lis2dh12<I2C> {
    ...
    #[inline]
    fn read_regs(&mut self, reg: Register, buffer: &mut [u8]) -> Result<(), I2C::Error> {
        self.i2c
            .write_read(self.addr, &[reg.addr() | I2C_SUB_MULTI], buffer)
    }

    #[inline]
    fn write_reg(&mut self, reg: Register, val: u8) -> Result<(), I2C::Error> {
        self.i2c.write(self.addr, &[reg.addr(), val])
    }

    #[inline]
    fn modify_reg<F>(&mut self, reg: Register, f: F) -> Result<(), I2C::Error>
    where
        F: FnOnce(u8) -> u8,
    {
    ...
    #[inline]
    fn reg_set_bits(&mut self, reg: Register, bits: u8) -> Result<(), I2C::Error> {
        self.modify_reg(reg, |v| v | bits)
    }

    #[inline]
    fn reg_reset_bits(&mut self, reg: Register, bits: u8) -> Result<(), I2C::Error> {
        self.modify_reg(reg, |v| v & !bits)
    }

    #[inline]
    fn reg_xset_bits(&mut self, reg: Register, bits: u8, set: bool) -> Result<(), I2C::Error> {
        if set {
    ...
    REG: IntRegs,
{
    ...
    /// Threshold,
    /// `INTx_THS`: `THS`
    pub fn set_ths(&mut self, ths: u8) -> Result<(), Error<I2C::Error>> {
    ...
    /// `INTx_THS`: `THS`
    #[cfg(feature = "out_f32")]
    pub fn set_thsf(&mut self, ths: f32) -> Result<(), Error<I2C::Error>> {
        self.set_ths(self.dev.fs.convert_ths_f32tou8(ths))
    }

    /// Duration,
    /// `INTx_DURATION`: `D`
    pub fn set_duration(&mut self, d: u8) -> Result<(), Error<I2C::Error>> {
